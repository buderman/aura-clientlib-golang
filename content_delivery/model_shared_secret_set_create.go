/*
content-delivery

Cotent Delivery API

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package content_delivery

import (
	"encoding/json"
)

// SharedSecretSetCreate Create one or more shared secrets for a content provider.  Shared secrets are used for URI signing, which is used as an authorization method for content delivery.
type SharedSecretSetCreate struct {
	ContentProviderId int32 `json:"contentProviderId"`
	Name string `json:"name"`
}

// NewSharedSecretSetCreate instantiates a new SharedSecretSetCreate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSharedSecretSetCreate(contentProviderId int32, name string) *SharedSecretSetCreate {
	this := SharedSecretSetCreate{}
	this.ContentProviderId = contentProviderId
	this.Name = name
	return &this
}

// NewSharedSecretSetCreateWithDefaults instantiates a new SharedSecretSetCreate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSharedSecretSetCreateWithDefaults() *SharedSecretSetCreate {
	this := SharedSecretSetCreate{}
	return &this
}

// GetContentProviderId returns the ContentProviderId field value
func (o *SharedSecretSetCreate) GetContentProviderId() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.ContentProviderId
}

// GetContentProviderIdOk returns a tuple with the ContentProviderId field value
// and a boolean to check if the value has been set.
func (o *SharedSecretSetCreate) GetContentProviderIdOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.ContentProviderId, true
}

// SetContentProviderId sets field value
func (o *SharedSecretSetCreate) SetContentProviderId(v int32) {
	o.ContentProviderId = v
}

// GetName returns the Name field value
func (o *SharedSecretSetCreate) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *SharedSecretSetCreate) GetNameOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *SharedSecretSetCreate) SetName(v string) {
	o.Name = v
}

func (o SharedSecretSetCreate) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["contentProviderId"] = o.ContentProviderId
	}
	if true {
		toSerialize["name"] = o.Name
	}
	return json.Marshal(toSerialize)
}

type NullableSharedSecretSetCreate struct {
	value *SharedSecretSetCreate
	isSet bool
}

func (v NullableSharedSecretSetCreate) Get() *SharedSecretSetCreate {
	return v.value
}

func (v *NullableSharedSecretSetCreate) Set(val *SharedSecretSetCreate) {
	v.value = val
	v.isSet = true
}

func (v NullableSharedSecretSetCreate) IsSet() bool {
	return v.isSet
}

func (v *NullableSharedSecretSetCreate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSharedSecretSetCreate(val *SharedSecretSetCreate) *NullableSharedSecretSetCreate {
	return &NullableSharedSecretSetCreate{value: val, isSet: true}
}

func (v NullableSharedSecretSetCreate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSharedSecretSetCreate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


